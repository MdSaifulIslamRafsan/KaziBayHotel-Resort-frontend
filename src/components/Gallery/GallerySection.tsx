...
const GallerySection = () => {
    const [activeTab, setActiveTab] = useState<'deluxe' | 'suite' | 'standard' | 'penthouse' | 'all'>('all');
    const [selectedImageIndex, setSelectedImageIndex] = useState<number | null>(null);


    const filteredImages = GALLERY_IMAGES.filter((img) => activeTab === "all" ? true : img.roomType === activeTab);
                ...
                <div className="flex flex-wrap gap-4 mb-5 border-b border-border">
                    {ROOM_TYPES?.map((room) => (
                        <button
                            ...
                            className={`pb-4 px-2 text-sm md:text-base font-medium transition-all relative ${activeTab === room.id
